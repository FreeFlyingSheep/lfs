...
当前版本 `0.12`，仅支持 `x86-64` 架构。注意，本项目纯粹是写着玩的，很多地方非常不规范。
...
若想在 QEMU 中加载构建完的 LFS，运行以下命令启动内存为 `1G` 的 QEMU 虚拟机：

    qemu-system-x86_64 -m 1024 lfs.img

若想在 Vmware 中运行，运行以下指令将虚拟磁盘转换成 vmdk 格式：
